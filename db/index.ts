...
async function createDatabaseConnection() {
  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    try {
      ...
      await sql`SELECT 1`;
      console.log(
        `Database connection established successfully after ${i + 1} attempts`,
      );
      return sql;
    } catch (error) {
      lastError = error;
  ...
  throw new Error(
    `Failed to establish database connection after ${MAX_RETRIES} attempts: ${lastError}`,
  );
}
...
export async function checkDatabaseConnection() {
  try {
    ...
    const result =
      await sql`SELECT NOW() as time, version() as version, current_database() as database`;
